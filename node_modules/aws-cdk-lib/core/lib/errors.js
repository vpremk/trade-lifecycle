...
`)}const THROWN_ERRORS=new Set;function maybeWriteErrorCode(errorCode){const file=process.env.CDK_ERROR_FILE;if(!file)return;const oldSize=THROWN_ERRORS.size;THROWN_ERRORS.add(errorCode),THROWN_ERRORS.size!==oldSize&&fs().writeFileSync(file,Array.from(THROWN_ERRORS).sort().join(`
`),"utf-8")}
